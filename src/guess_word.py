...
def set_space(only_len=False) -> int:
    ...
    if only_len:
        return word_len
    if word_len <= 5:
        return 5 - (word_len / 5) # type: ignore
    elif word_len <= 10:
        return 5 - (word_len / 5 + 0.5) # type: ignore
    else:
        return 5 - (word_len / 5 + 1) # type: ignore


def load_box():
    word = [c if c in st.session_state.guessed else '_' for c in st.session_state.word]
    for i in range(set_space(True)):
        guess_box[i + 1].button(word[i], key=f"guess_{i}")  # Use 'guess_' prefix to make the keys unique
...
def main():
    ...
    guess = placeholder.text_input(
        'Guess a letter', key=f"input_{st.session_state.input}", max_chars=1
    ).lower()  # Use 'input_' prefix to make the key unique
